...
#Version 6 (18 Sep 2012) allows for top x% of data to be plotted only (useful for large datasets)
...
#qq.plot(p)
#
qq.plot <- function( x, alpha=0.05, datatype="pvalue", scaletype="pvalue", df=1, plot.concentration.band=TRUE, one.sided=FALSE, frac=1, print=FALSE, xat=NULL, yat=NULL, main=NULL, xlab=NULL, ylab=NULL, pch="x", cex=0.5, col="black", ... )
{
  ...
  a <- (starti:n)                           #indices of the x points to be plotted, in forwards order
  lena = length(a)
  a2 <- (1:lena)                       #indices with same length as a, starting at 1
  ...
  if (scaletype=="pvalue") {                                              #Note scaletype forced="quantile" for stdnorm data, so here all data is on chisq scale
    if (!is.null(xat)) x4Lx=xat  else x4Lx = pretty( -log10( pchisq(c(E[1],E[lena]),df=df,lower.tail=FALSE) ) )
    if (!is.null(yat)) y4Ly=yat  else y4Ly = pretty( -log10( pchisq(c(O[1],O[lena]),df=df,lower.tail=FALSE) ) )
    ...
    if (!is.null(xat)) xnums=xat else xnums=pretty(c(E[1],E[lena]))
    if (!is.null(yat)) ynums=yat else ynums=pretty(c(O[1],O[lena]))
  ...
  plot( c(E[1],E[lena]), c(O[1],O[lena]), main = main, xlab = xlab, ylab = ylab, type = "n", xaxt = "n", yaxt = "n", ... )       #Just plots the outside box
  ...
  if (plot.concentration.band==TRUE) {
    if (one.sided==FALSE) {
      ...
      upper <- rep(1,lena)               #Exp. upper CL for 'a'th U(0,1) order statistic (becomes 'lower')
