...
option_list <- list(
  ...
  make_option("--width", type="numeric", default=8.5,
    help="Width QQ plot in inch [default=8]"),
  make_option("--height", type="numeric", default=8.5,
    help="Height QQ plot in inch [default=8]"),
  ...
  make_option("--maf", type="character", default="MAF",
    help="name of column with MAF, here: fraction with burden [default='MAF']"),
...
legendtext <- paste0("Fraction with Rare Variants=",fbin,"; N Genes=",format(fN,big.mark=",",scientific=FALSE))

## QQ plot by binned frequencies
pdf(file = paste0(opt$prefix,"_QQ.pdf"), width = opt$width, height = opt$height, pointsize = opt$pointsize)
